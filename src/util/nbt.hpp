...
namespace valk::nbt {
    ...
    class Root {
    public:
        template<typename T> requires std::is_base_of_v<Root, T>
        T* as() noexcept {
            return dynamic_cast<T*>(this);
        }
    };

    template<TagType T, bool Owned = true>
    class List : public Root {
    public:
        // If Copied == true, use the owned variant of the tag, otherwise use ref.
        using StorageT = std::conditional_t<Owned, typename TagMeta<T>::OwnedT, typename TagMeta<T>::RefT>;
        ...
        StorageT& operator[](size_t index) {
            return m_values[index];
        }
        ...
        const StorageT& at(size_t index) const {
            // checking bounds manually instead of using .at() because `m_values` might be a pointer.
            if (index >= size()) {
    ...
    template<TagType U, bool O>
    struct TagMeta<List<U, O>> {
    ...
    public:
        ...
        template<TagType T>
        std::add_const_t<std::add_lvalue_reference_t<typename TagMeta<T>::RefT>> get(const char* key) const {
    ...
    struct ByteArray : List<Byte, Owned> {};

    template<bool O>
    struct TagMeta<ByteArray<O>> {
    ...
    template<bool Owned = false>
    struct IntArray : List<Int, Owned> {};

    template<bool O>
    struct TagMeta<IntArray<O>> {
    ...
    template<bool Owned = false>
    struct LongArray : List<Long, Owned> {};

    template<bool O>
    struct TagMeta<LongArray<O>> {
    ...
    ValkExpect<Root> deserialize(detail::BinarySpan<SpanOwned> span) {
